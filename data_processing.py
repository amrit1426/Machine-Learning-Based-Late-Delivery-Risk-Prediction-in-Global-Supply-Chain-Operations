...
# ================================================================
#
#  DEPLOYMENT NOTE (2025):
#  This file is imported by app.py purely so that pickle can resolve
#  module references stored inside the trained artifacts.
#  All training execution is guarded with `if __name__ == "__main__":`
#  so that `import data_processing` is safe in the Streamlit Cloud
#  environment where APL_Logistics.csv does not exist.
#
#  To retrain:  python data_processing.py
#  To serve:    streamlit run app.py
# ================================================================
...
import pandas as pd
import matplotlib
matplotlib.use("Agg")           # headless backend — safe for import in cloud
...
# Only create directories when running as a script, not when imported.
# On Streamlit Cloud this would otherwise try to create dirs in a
# read-only filesystem location.
if __name__ != "__main__":
    pass   # imported — do nothing
else:
...
# ----------------------------------------------------------------
# FEATURE ENGINEERING (module-level — available on import)
# ----------------------------------------------------------------
# This function is defined at module scope (outside the __main__
# guard) so that:
#   1. app.py can call it directly during inference.
#   2. If any pickle artifact stores a reference to
#      data_processing.engineer_features, it can be resolved.
# The function body is IDENTICAL to the original — no logic change.
# ----------------------------------------------------------------
...
if __name__ == "__main__":
    ...
    # ----------------------------------------------------------------
    # engineer_features() is defined at module scope above.
